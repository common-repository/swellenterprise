			...
			if ( $this->field['options']['repeater'] ) {
				...
				if ( $this->value ) {
					...
					foreach ( $this->value as $key => $value ) {
						...
						echo esc_html('<h4 class="exopite-sof-cloneable__title exopite-sof-accordion__title"><span class="exopite-sof-cloneable__text">') . esc_attr($this->field['options']['group_title']) . esc_html('</span>');
						echo esc_html( '<span class="exopite-sof-cloneable--helper">' );
						if ( $sortable ) {
							echo esc_html( '<i class="fa fa-arrows-v"></i>' );
						}
